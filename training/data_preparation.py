...
def load_nasa_data(data_dir: Path, years: list = None, filter_ontario: bool = True) -> pd.DataFrame:
    ...
    if filter_ontario:
        ...
        print(f"[OK] Ontario fires: {filtered_count:,} ({filtered_count/original_count*100:.1f}% of total)")
...
def generate_training_samples(fires_df: pd.DataFrame, samples_per_fire: int = 20) -> pd.DataFrame:
    ...
    print(f"\n[OK] Generated {len(df):,} training samples")
...
def prepare_features(samples_df: pd.DataFrame) -> tuple:
    ...
    print(f"[OK] Features: {', '.join(FEATURE_NAMES[:5])}... (+6 more)")
